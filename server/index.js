...
await query(`CREATE TABLE IF NOT EXISTS password_reset_requests(id BIGSERIAL PRIMARY KEY,user_type VARCHAR(20) NOT NULL CHECK(user_type IN ('customer','user')),customer_user_id BIGINT,user_id BIGINT,identifier VARCHAR(254) NOT NULL,token_hash TEXT,status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','approved','rejected','used','expired')),expires_at TIMESTAMPTZ,requested_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),approved_at TIMESTAMPTZ,approved_by BIGINT)`);
await query(`CREATE INDEX IF NOT EXISTS idx_password_reset_requests_status ON password_reset_requests(status, requested_at DESC)`);
...
function appBaseUrl(req){return String(process.env.PUBLIC_APP_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/,'')}
function resetUrl(req,type,token){return `${appBaseUrl(req)}/reset-password?type=${encodeURIComponent(type)}&token=${encodeURIComponent(token)}`}
async function seed() {
...
app.post('/api/customer/forgot-password',loginLimiter,async(req,res,next)=>{try{const identifier=String(req.body?.identifier||'').trim();if(identifier.length>254)return res.status(400).json({error:'البيانات غير صالحة'});const q=await query('SELECT id FROM customer_users WHERE phone=$1 OR LOWER(email)=LOWER($1) LIMIT 1',[identifier]);if(q.rows[0]){await query("UPDATE password_reset_requests SET status='expired' WHERE user_type='customer' AND customer_user_id=$1 AND status IN ('pending','approved')",[q.rows[0].id]);await query('INSERT INTO password_reset_requests(user_type,customer_user_id,identifier) VALUES($1,$2,$3)',['customer',q.rows[0].id,identifier]);}res.json({ok:true,message:'تم إرسال طلب إعادة تعيين كلمة المرور إلى الإدارة. سيتم السماح بإعادة التعيين بعد الموافقة.'})}catch(e){next(e)}});
app.post('/api/customer/reset-password',loginLimiter,async(req,res,next)=>{try{const token=String(req.body?.token||''),password=String(req.body?.password||'');if(token.length<40||password.length<8||password.length>200)return res.status(400).json({error:'رابط إعادة التعيين أو كلمة المرور غير صالح'});const q=await query("SELECT * FROM password_reset_requests WHERE user_type='customer' AND token_hash=$1 AND status='approved' AND expires_at>NOW() ORDER BY id DESC LIMIT 1",[resetTokenHash(token)]);const r=q.rows[0];if(!r)return res.status(400).json({error:'رابط إعادة التعيين غير صالح أو منتهي'});await query('UPDATE customer_users SET password_hash=$1 WHERE id=$2',[bcrypt.hashSync(password,12),r.customer_user_id]);await query("UPDATE password_reset_requests SET status='used' WHERE id=$1",[r.id]);res.json({ok:true,message:'تم تغيير كلمة المرور بنجاح.'})}catch(e){next(e)}});
...
app.post('/api/auth/forgot-password',loginLimiter,async(req,res,next)=>{try{const email=String(req.body?.email||'').trim().toLowerCase();if(!email||email.length>254)return res.json({ok:true,message:'تم إرسال طلب إعادة تعيين كلمة المرور إلى المدير. سيتم السماح بإعادة التعيين بعد الموافقة.'});const q=await query('SELECT id FROM users WHERE LOWER(email)=LOWER($1) LIMIT 1',[email]);if(q.rows[0]){await query("UPDATE password_reset_requests SET status='expired' WHERE user_type='user' AND user_id=$1 AND status IN ('pending','approved')",[q.rows[0].id]);await query('INSERT INTO password_reset_requests(user_type,user_id,identifier) VALUES($1,$2,$3)',['user',q.rows[0].id,email]);}res.json({ok:true,message:'تم إرسال طلب إعادة تعيين كلمة المرور إلى المدير. سيتم السماح بإعادة التعيين بعد الموافقة.'})}catch(e){next(e)}});
app.post('/api/auth/reset-password',loginLimiter,async(req,res,next)=>{try{const token=String(req.body?.token||''),password=String(req.body?.password||'');if(token.length<40||password.length<10||password.length>200)return res.status(400).json({error:'رابط إعادة التعيين أو كلمة المرور غير صالح'});const q=await query("SELECT * FROM password_reset_requests WHERE user_type='user' AND token_hash=$1 AND status='approved' AND expires_at>NOW() ORDER BY id DESC LIMIT 1",[resetTokenHash(token)]);const r=q.rows[0];if(!r)return res.status(400).json({error:'رابط إعادة التعيين غير صالح أو منتهي'});await query('UPDATE users SET password_hash=$1 WHERE id=$2',[bcrypt.hashSync(password,12),r.user_id]);await query("UPDATE password_reset_requests SET status='used' WHERE id=$1",[r.id]);res.json({ok:true,message:'تم تغيير كلمة المرور بنجاح.'})}catch(e){next(e)}});
...
app.get('/api/admin/password-reset-requests',auth,requireAdmin,async(req,res,next)=>{try{const q=await query(`SELECT r.id,r.user_type,r.identifier,r.requested_at,r.status,r.expires_at,r.customer_user_id,r.user_id,COALESCE(c.name,u.name) AS name,COALESCE(c.phone,u.email) AS contact FROM password_reset_requests r LEFT JOIN customer_users c ON r.customer_user_id=c.id LEFT JOIN users u ON r.user_id=u.id WHERE r.status='pending' ORDER BY r.id DESC LIMIT 50`);res.json({requests:q.rows.map(r=>({...r,id:Number(r.id),customer_user_id:r.customer_user_id==null?null:Number(r.customer_user_id),user_id:r.user_id==null?null:Number(r.user_id)}))})}catch(e){next(e)}});
app.post('/api/admin/password-reset-requests/:id/approve',auth,requireCsrf,requireAdmin,async(req,res,next)=>{try{const id=Number(req.params.id);if(!Number.isInteger(id)||id<1)return res.status(400).json({error:'طلب غير صالح'});const q=await query("SELECT * FROM password_reset_requests WHERE id=$1 AND status='pending'",[id]);const r=q.rows[0];if(!r)return res.status(404).json({error:'طلب إعادة التعيين غير موجود أو تمت معالجته'});const token=crypto.randomBytes(32).toString('hex');const approved=await query("UPDATE password_reset_requests SET status='approved',token_hash=$1,expires_at=NOW()+INTERVAL '15 minutes',approved_at=NOW(),approved_by=$2 WHERE id=$3 AND status='pending' RETURNING id",[resetTokenHash(token),req.user.id,id]);if(!approved.rows[0])return res.status(409).json({error:'تمت معالجة طلب إعادة التعيين من مدير آخر.'});await audit(req,'APPROVE_PASSWORD_RESET',r.user_type,r.customer_user_id||r.user_id,`request=${id}`);res.json({ok:true,resetUrl:resetUrl(req,r.user_type,token),expiresInMinutes:15,userType:r.user_type})}catch(e){next(e)}});
...
app.get('/api/admin/data', auth, async (req, res, next) => {
  try {
    ...
    const [categories, items, offers, orders, users, customers, settings, zones, coupons, reviews, resetRequests] = await Promise.all([
      ...
      canUsers ? query(`SELECT r.id,r.user_type,r.identifier,r.requested_at,r.status,r.expires_at,r.customer_user_id,r.user_id,COALESCE(c.name,u.name) AS name,COALESCE(c.phone,u.email) AS contact FROM password_reset_requests r LEFT JOIN customer_users c ON r.customer_user_id=c.id LEFT JOIN users u ON r.user_id=u.id WHERE r.status='pending' ORDER BY r.id DESC LIMIT 50`) : Promise.resolve({ rows: [] })
    ]);
    ...
    res.json({
      ...
      resetRequests: resetRequests.rows.map(r => ({ ...r, id:Number(r.id), customer_user_id:r.customer_user_id==null?null:Number(r.customer_user_id), user_id:r.user_id==null?null:Number(r.user_id) }))
    });
  } catch (e) { next(e); }
});
